(function() {
    ...
    $(function() {
        ...
        $(document).ready(function() {
          // var standardTags = ['facebook', 'twitter', 'pinterest'];
          // localStorage.setItem('rumin_tags', JSON.stringify(standardTags));
            ...
            var tagsSelect = new Choices(
                '#choices-multiple-tags', {
                  ...
                  choices: existing_tags.map(function(tag) {
                      return { label: tag, value: tag };
                    })
                }
            );
          
            var addTags = new Choices(
                document.getElementById('choices-text-remove-button-add-tag'), {
                    delimiter: ',',
                    editItems: true,
                    maxItemCount: 5,
                    removeItemButton: true,
            ...
            addTags.passedElement.element.addEventListener(
              'addItem',
              function(event) {
                let tags = JSON.parse(localStorage.getItem('rumin_tags'));
                console.log(tags);

                const new_tags = [event.detail.value];
                console.log(new_tags);

                // credit: [Creating unique, merged arrays using JavaScript's Set (and more) | Jamstack developer | Rob Kendal](https://robkendal.co.uk/blog/2020-02-04-creating-unique-merged-arrays-using-javascripts-set-and-more)
                var merged_tags = [...new Set([...tags,...new_tags])]
                console.log(merged_tags);

                localStorage.setItem('rumin_tags', JSON.stringify(merged_tags));
              }
            );
        });
